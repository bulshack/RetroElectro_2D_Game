...
public class AR_Location : MonoBehaviour
{
    ...
    //Debugging //text box on screen with distance as a double
    ...
    public void Calc(float lat1, float lon1, float lat2, float lon2)
    {
        ...
        //Debugging //set the distance text on the canvas
    ...
    void Start()
    {
        //Debugging //get distance text reference
        ...
        if (mazeObject.activeInHierarchy)
        {

            //start GetCoordinate() function 
            StartCoroutine("GetCoordinates");
            //initialize target and original position
            targetPosition = mazeObject.transform.position;
            originalPosition = mazeObject.transform.position;
        }
    }
    ...
    void Update()
    {
        ...
        mazeObject.transform.position = Vector3.Lerp(transform.position, targetPosition, speed);
        
        //Debugging_Optional //rotate by 1 degree about the y axis every frame
        //distanceTextObject.transform.eulerAngles += new Vector3(0, 1f, 0);
    }
}
